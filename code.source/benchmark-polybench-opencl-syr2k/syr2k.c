...
{
	a_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE, N*M*sizeof(DATA_TYPE), NULL, &err_code);
	b_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE, N*M*sizeof(DATA_TYPE), NULL, &err_code);
	c_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE, N*M*sizeof(DATA_TYPE), NULL, &err_code);
	
	if(err_code != CL_SUCCESS) printf("Error in creating buffers\n");

	err_code = clEnqueueWriteBuffer(clCommandQue, a_mem_obj, CL_TRUE, 0, N*M*sizeof(DATA_TYPE), A, 0, NULL, NULL);
	err_code = clEnqueueWriteBuffer(clCommandQue, b_mem_obj, CL_TRUE, 0, N*M*sizeof(DATA_TYPE), B, 0, NULL, NULL);
	err_code = clEnqueueWriteBuffer(clCommandQue, c_mem_obj, CL_TRUE, 0, N*M*sizeof(DATA_TYPE), C, 0, NULL, NULL);
...
void cl_load_prog()
{
	...
	clKernel1 = clCreateKernel(clProgram, "syr2k_kernel", &err_code);
	if(err_code != CL_SUCCESS) 
        {
...
//	t_start = rtclock();
	...
	err_code |= clSetKernelArg(clKernel1, 3, sizeof(DATA_TYPE), (void *)&ALPHA);
	err_code |= clSetKernelArg(clKernel1, 4, sizeof(DATA_TYPE), (void *)&BETA);
	err_code |= clSetKernelArg(clKernel1, 5, sizeof(int), (void *)&m);
	err_code |= clSetKernelArg(clKernel1, 6, sizeof(int), (void *)&n);
	
	if(err_code != CL_SUCCESS)
        {
          printf("Error in setting arguments1\n");
          exit(1);
        }
