...
void cl_mem_init(DATA_TYPE *A, DATA_TYPE *B, DATA_TYPE *C, DATA_TYPE *D, DATA_TYPE *E)
{
	...
	d_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE, sizeof(DATA_TYPE) * NJ * NL, NULL, &err_code);
	e_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE, sizeof(DATA_TYPE) * NI * NL, NULL, &err_code);
	...
	err_code = clEnqueueWriteBuffer(clCommandQue, d_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * NJ * NL, D, 0, NULL, NULL);
	err_code = clEnqueueWriteBuffer(clCommandQue, e_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * NI * NL, E, 0, NULL, NULL);
...
void cl_load_prog()
{
	...
	clKernel1 = clCreateKernel(clProgram, "mm2_kernel1", &err_code);
	if(err_code != CL_SUCCESS) 
        {
	...
	clKernel2 = clCreateKernel(clProgram, "mm2_kernel2", &err_code);
	if(err_code != CL_SUCCESS) 
        {
...
//	t_start = rtclock();
	...
	err_code |= clSetKernelArg(clKernel1, 2, sizeof(cl_mem), (void *)&c_mem_obj);
	err_code |= clSetKernelArg(clKernel1, 3, sizeof(int), (void *)&ni);
	err_code |= clSetKernelArg(clKernel1, 4, sizeof(int), (void *)&nk);
	err_code |= clSetKernelArg(clKernel1, 5, sizeof(int), (void *)&nj);
	if(err_code != CL_SUCCESS)
        {
	...
	err_code |= clSetKernelArg(clKernel2, 2, sizeof(cl_mem), (void *)&e_mem_obj);
	err_code |= clSetKernelArg(clKernel2, 3, sizeof(int), (void *)&ni);
	err_code |= clSetKernelArg(clKernel2, 4, sizeof(int), (void *)&nj);
	err_code |= clSetKernelArg(clKernel2, 5, sizeof(int), (void *)&nl);
	if(err_code != CL_SUCCESS)
        {
          printf("Error in seting arguments\n");
          exit(1);
...
int main(void) 
{
  ...
  for (ct_repeat=0; ct_repeat<ct_repeat_max; ct_repeat++)
  {
    ...
    err_code = clEnqueueReadBuffer(clCommandQue, e_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * NI * NL, E_outputFromGpu, 0, NULL, NULL);
    if(err_code != CL_SUCCESS)
    {
