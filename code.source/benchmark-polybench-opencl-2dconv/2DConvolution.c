...
{
	a_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_ONLY, sizeof(DATA_TYPE) * NI * NJ, NULL, &err_code);
	b_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE, sizeof(DATA_TYPE) * NI * NJ, NULL, &err_code);
	
	if(err_code != CL_SUCCESS) printf("Error in creating buffers\n");

	err_code = clEnqueueWriteBuffer(clCommandQue, a_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * NI * NJ, A, 0, NULL, NULL);
...
void cl_load_prog()
{
	...
	clKernel = clCreateKernel(clProgram, "Convolution2D_kernel", &err_code);
	if(err_code != CL_SUCCESS) 
        {
...
int main(int argc, char *argv[])
{
  ...
  for (ct_repeat=0; ct_repeat<ct_repeat_max; ct_repeat++)
  {
    ...
    err_code = clEnqueueReadBuffer(clCommandQue, b_mem_obj, CL_TRUE, 0, NI*NJ*sizeof(DATA_TYPE), B_outputFromGpu, 0, NULL, NULL);
    if(err_code != CL_SUCCESS)
    {
